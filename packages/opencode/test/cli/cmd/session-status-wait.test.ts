...
import { deriveBusyFromLatestMessage, extractNewParts, partWaitKey, waitLoop } from "../../../src/cli/cmd/session"
...
function textPart(id: string, text = "hi"): MessageV2.TextPart {
  return {
    id: PartID.make(id),
    sessionID: sid,
    messageID: mid,
...
function bashTool(id: string, status: "pending" | "running" | "completed", command = "ls"): MessageV2.ToolPart {
...
function stepStart(id: string): MessageV2.Part {
  return {
    id: PartID.make(id),
    sessionID: sid,
    messageID: mid,
...
describe("extractNewParts", () => {
  test("returns oldest→newest, drops non-informative, dedupes by key", () => {
    const recent: MessageV2.Part[] = [bashTool("p3", "completed"), stepStart("p2"), bashTool("p1", "running")]
...
describe("waitLoop", () => {
  type Tick = {
    dv: number
    snap: any
    parts?: { part: MessageV2.Part; timeUpdated: number }[]
  }
