...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# clickzetta") || content.includes(".clickzetta/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".clickzetta/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".clickzetta/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".clickzetta"))
    ) {
      continue
    }
