...
import { readLlmEntries, writeLlmEntries, type LlmEntryView } from "../../clickzetta/native-config"
...
// cz_change: LLM config lives in ~/.clickzetta/llm.json (opencode native format),
// connection profiles stay in profiles.toml. readProfiles merges them into the
// command's working view; writeProfiles splits them back to the right files.
function readProfiles(): TomlRecord {
  let data: TomlRecord = {}
  if (existsSync(PROFILES_PATH)) {
    try {
      const parsed = parseToml(readFileSync(PROFILES_PATH, "utf-8"))
      if (isRecord(parsed)) {
        data = parsed
        if (migrateLegacyClickzettaConfig(parsed)) writeProfilesToml(parsed)
      }
    } catch {}
  }
  // llm + default_llm come from llm.json, not profiles.toml
  const { llm, default_llm } = readLlmEntries()
  delete data.llm
  delete data.default_llm
  if (Object.keys(llm).length) data.llm = llm as unknown as TomlRecord
  if (default_llm) data.default_llm = default_llm
  return data
}

function writeProfilesToml(data: TomlRecord): void {
...
function writeProfiles(data: TomlRecord): void {
  // Split: llm/default_llm → llm.json; everything else → profiles.toml.
  const llm = isRecord(data.llm) ? (data.llm as Record<string, LlmEntryView>) : {}
  const default_llm = typeof data.default_llm === "string" ? data.default_llm : undefined
  writeLlmEntries({ llm, ...(default_llm && { default_llm }) })
  const toml: TomlRecord = { ...data }
  delete toml.llm
  delete toml.default_llm
  writeProfilesToml(toml)
}
