...
function writeProfile(cred: Credential, profileName: string): void {
  ...
  const lines: string[] = []
  lines.push(`default_profile = "${profileName}"`)
  lines.push("")
  lines.push(`[profiles.${profileName}]`)
  if (cred.instanceName) lines.push(`instance = "${cred.instanceName}"`)
  if (cred.workspaceName) lines.push(`workspace = "${cred.workspaceName}"`)
  if (cred.schema) lines.push(`schema = "${cred.schema}"`)
  if (cred.virtualCluster) lines.push(`vcluster = "${cred.virtualCluster}"`)
  if (cred.accessToken) lines.push(`pat = "${cred.accessToken}"`)
  if (cred.service) lines.push(`service = "${cred.service}"`)
  if (cred.service?.startsWith("https")) lines.push(`protocol = "https"`)
  else if (cred.service) lines.push(`protocol = "http"`)
  if (cred.username) lines.push(`username = "${cred.username}"`)
  if (cred.apiKey) lines.push(`api_key = "${cred.apiKey}"`)
  if (cred.aimeshEndpointBaseUrl) lines.push(`aimesh_endpoint = "${cred.aimeshEndpointBaseUrl}"`)
  lines.push("")
