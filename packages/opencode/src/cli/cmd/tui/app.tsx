...
function rendererConfig(_config: TuiConfig.Info): CliRendererConfig {
  const mouseEnabled = !Flag.CLICKZETTA_DISABLE_MOUSE && (_config.mouse ?? true)
...
function App(props: { onSnapshot?: () => Promise<string[]> }) {
  ...
  createEffect(() => {
    if (!terminalTitleEnabled() || Flag.CLICKZETTA_DISABLE_TERMINAL_TITLE) return
    ...
    if (route.data.type === "session") {
      ...
      renderer.setTerminalTitle(`CZ | ${title}`)
      return
    }

    if (route.data.type === "plugin") {
      renderer.setTerminalTitle(`CZ | ${route.data.id}`)
    }
  })
  ...
  return (
    <box
      ...
      onMouseUp={Flag.CLICKZETTA_EXPERIMENTAL_DISABLE_COPY_ON_SELECT ? undefined : () => Selection.copy(renderer, toast)}
    >
      <Show when={Flag.CLICKZETTA_SHOW_TTFD}>
        <TimeToFirstDraw />
      </Show>
      <Show when={ready()}>
        <Switch>
