...
// TODO(czcli): Add ClickZetta subscription dialog
...
export function Session() {
  ...
  // TODO(czcli): Add ClickZetta subscription upsell handler here
  ...
  createEffect(() => {
    ...
    const blue = UI.Style.TEXT_INFO_BOLD
    return exit.message.set(
      [
        ...
        `  ${dim}Continue${reset} ${bold}cz-cli agent -s ${session()?.id}${reset}`,
        ``,
      ].join("\n"),
    )
  })
  ...
  return (
    <Show when={props.part.text.trim()}>
      <box id={"text-" + props.part.id} paddingLeft={3} marginTop={1} flexShrink={0}>
        <Switch>
          <Match when={Flag.CLICKZETTA_EXPERIMENTAL_MARKDOWN}>
            <markdown
              syntaxStyle={syntax()}
              streaming={true}
