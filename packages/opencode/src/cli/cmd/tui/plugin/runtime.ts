...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".clickzetta"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".clickzetta", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".clickzetta", "tui.json"),
  }
}
...
async function load(input: { api: Api; config: TuiConfig.Info }) {
  ...
  try {
    await Instance.provide({
      directory: cwd,
      fn: async () => {
        const records = Flag.CLICKZETTA_PURE ? [] : (config.plugin_origins ?? [])
        if (Flag.CLICKZETTA_PURE && config.plugin_origins?.length) {
