...
import { rewriteClickzettaGatewayError } from "@clickzetta/cli/llm/gateway-error"

/**
 * @clickzetta/ai-provider — a thin shell over @ai-sdk/openai-compatible for the
 ...
 */
function wrapModel(model: LanguageModelV3): LanguageModelV3 {
...
export function createClickzetta(options: ClickzettaProviderSettings): OpenAICompatibleProvider {
  ...
  const languageModel = (modelId: string): LanguageModelV3 => wrapModel(base.languageModel(modelId))

  const provider = ((modelId: string) => languageModel(modelId)) as OpenAICompatibleProvider
  provider.languageModel = languageModel
  provider.chatModel = (modelId: string) => wrapModel(base.chatModel(modelId))
