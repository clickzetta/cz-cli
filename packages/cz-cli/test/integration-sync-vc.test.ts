...
// cz_change: upstream mkdtemp'd a PRIVATE home here and deleted
// CLICKZETTA_TEST_HOME in afterAll. a2 wires test/preload.ts via bunfig.toml,
// which owns HOME/CLICKZETTA_TEST_HOME for the whole process; unsetting it left
// later files (task-lineage, task-condition-flow, ai-gateway-*) resolving the
// developer's real home and failing with NO_PROFILE. Write the profile into the
// preload's shared home instead, and leave the env alone.
...
afterAll(() => {
  // cz_change: bun's mock.module registrations are process-global and outlive
  // this file. a2's other cz-cli suites are network-boundary tests that run the
  // REAL modules and stub only globalThis.fetch, so leftover mocks here silently
  // break them (task-lineage, task-condition-flow, ai-gateway-*). Restore every
  // module this file replaced — same guard as clickzetta-sdk/test/dqc.test.ts.
  mock.module("@clickzetta/sdk", () => actualSdk)
  mock.module("../src/commands/studio-context.js", () => actualStudioContext)
  mock.module("../src/commands/datasource.js", () => actualDatasource)
  mock.module("../src/resolver.js", () => actualResolver)
  mock.module("../src/logger.js", () => actualLogger)
  mock.module("../src/commands/studio-url.js", () => actualStudioUrl)
})
