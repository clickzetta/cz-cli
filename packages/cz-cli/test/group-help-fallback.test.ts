...
// plain cz-cli path (no agent-runtime delegation). Each, invoked bare, must
...
describe("bare command group renders help (exit 0), not USAGE_ERROR", () => {
  ...
  test("cz-cli mcp help lists its serve subcommand", () => {
    const r = run(["mcp"])
    expect(r.exitCode).toBe(0)
...
describe("genuine errors still surface (not masked by help fallback)", () => {
  test("mcp unknown subcommand -> USAGE_ERROR exit 2", () => {
    const r = run(["mcp", "serv"])
...
describe("programmatic execute() boundary renders bare-group help", () => {
  ...
  test("execute('mcp') returns help text and exit 0", async () => {
    const { exitCode, output } = await execute("mcp")
    expect(exitCode).toBe(0)
    expect(output).toContain("cz-cli mcp")
    expect(output).toContain("Commands:")
    expect(output).not.toContain("USAGE_ERROR")
  })
