  ...
  return withClickZettaProfileOption(yargs(coalesceJsonArrayOptionArgs(args)))
    ...
    .fail((msg, err, yargs) => {
      ...
      const message = (msg && msg.trim() !== "") ? msg : (() => {
        ...
        const KNOWN_COMMANDS = new Set(["sql", "schema", "table", "workspace", "status", "login", "profile", "task", "runs", "attempts", "job", "agent", "serve", "setup", "update", "datasource", "ai-gateway", "analytics-agent"])
