...
export function registerStaticCommands(): void {
  registerCommands([
    ...
    { name: "table load", kind: "command", description: "Load a czfs Volume path into a table (directory sources must end with /); use SQL for advanced COPY INTO/OVERWRITE cases",
      ...
      options: [{ flags: "--using", required: false, takes_value: true, help: "Input format: csv, parquet, orc, bson (default csv)" }, { flags: "--header", required: false, takes_value: false, help: "Treat the first CSV row as column names" }, { flags: "--write", required: true, takes_value: false, help: "Confirm loading into the target table" }],
