...
export function buildAiGuide(options?: { wide?: boolean; budgetChars?: number }): GuidePayload {
  ...
  const payload: Record<string, unknown> = {
    ...
    safety: {
      write_protection: "Start SQL and guarded schema/table/fs operations without --write. On action_required, ask the user to approve and wait before following issues[].remediation. Never automatically retry to bypass a guard. DELETE/UPDATE without WHERE remain blocked.",
  ...
  registerCommands([
    { name: "sql", kind: "command", description: "Execute SQL against ClickZetta",
      ...
      examples: [
        ...
        { cmd: "cz-cli sql -f query.sql", desc: "Run SQL from file; if action_required is returned, ask the user to approve before retrying" },
      ],
    },
    { name: "fs", kind: "group", description: "Manage local files and Lakehouse Volume files" },
    { name: "fs ls", kind: "command", description: "List files or directories",
      arguments: [{ name: "path", required: true }],
      options: [{ flags: "-R, --recursive", required: false, takes_value: false, help: "Include files in subdirectories" }, { flags: "--limit", required: false, takes_value: true, help: "Maximum entries (0 for unlimited)" }],
    ...
    { name: "fs mb", kind: "command", description: "Create a Managed Volume only; cannot create User or Table Volumes",
      options: [{ flags: "--write", required: false, takes_value: false, help: "Allow only after explicit user approval" }],
      arguments: [{ name: "volume", required: true }],
      examples: [{ cmd: "cz-cli fs mb czfs:/Volumes/your_workspace/your_schema/your_volume", desc: "Create a Managed Volume" }] },
    { name: "fs rb", kind: "command", description: "Remove an empty Managed Volume object with --write; does not remove files",
      options: [{ flags: "--write", required: false, takes_value: false, help: "Allow only after explicit user approval" }],
      arguments: [{ name: "volume", required: true }],
      examples: [{ cmd: "cz-cli fs rb czfs:/Volumes/your_workspace/your_schema/your_volume", desc: "Drop the Managed Volume object" }] },
    { name: "fs mkdir", kind: "command", description: "Create a directory and missing parents inside an existing filesystem or Volume",
      arguments: [{ name: "path", required: true }],
      examples: [{ cmd: "cz-cli fs mkdir czfs:/Volumes/your_workspace/your_schema/your_volume/data/2026/08", desc: "Create directories in a Managed Volume" }, { cmd: "cz-cli fs mkdir czfs:/Volumes/@user/your_workspace/your_user/data/2026/08", desc: "Create directories in a User Volume" }, { cmd: "cz-cli fs mkdir czfs:/Volumes/@table/your_workspace/your_schema/your_table/data/2026/08", desc: "Create directories in a Table Volume" }] },
    { name: "fs cp", kind: "command", description: "Copy a file or directory",
      arguments: [{ name: "source", required: true }, { name: "destination", required: true }],
      options: [{ flags: "-R, --recursive", required: false, takes_value: false, help: "Copy files in subdirectories" }, { flags: "--overwrite/--no-overwrite", required: false, takes_value: false, help: "Replace existing destination files (default: refuse)" }],
      examples: [{ cmd: "cz-cli fs cp ./data.csv czfs:/Volumes/your_workspace/your_schema/your_volume/data.csv", desc: "Upload a local file" }, { cmd: "cz-cli fs cp czfs:/Volumes/your_workspace/your_schema/your_volume/data.csv ./downloads/data.csv", desc: "Download a Volume file" }] },
    { name: "fs mv", kind: "command", description: "Move a file or directory",
      arguments: [{ name: "source", required: true }, { name: "destination", required: true }],
      options: [{ flags: "-R, --recursive", required: false, takes_value: false, help: "Move files in subdirectories" }, { flags: "--overwrite/--no-overwrite", required: false, takes_value: false, help: "Replace existing destination files (default: refuse)" }],
      examples: [{ cmd: "cz-cli fs mv czfs:/Volumes/your_workspace/your_schema/your_volume/inbox/data.csv czfs:/Volumes/your_workspace/your_schema/your_volume/archive/data.csv", desc: "Move a Volume file" }] },
    { name: "fs rm", kind: "command", description: "Remove a file or directory",
      arguments: [{ name: "path", required: true }],
      options: [{ flags: "-R, --recursive", required: false, takes_value: false, help: "Remove directories recursively" }, { flags: "-f, --force", required: false, takes_value: false, help: "Ignore missing paths" }, { flags: "--dry-run", required: false, takes_value: false, help: "Preview without deleting" }, { flags: "--write", required: false, takes_value: false, help: "Allow removal only after explicit user approval" }],
      examples: [{ cmd: "cz-cli fs rm czfs:/Volumes/your_workspace/your_schema/your_volume/tmp/data.csv", desc: "Remove one file" }, { cmd: "cz-cli fs rm czfs:/Volumes/your_workspace/your_schema/your_volume/tmp/ -R --dry-run", desc: "Preview recursive removal" }] },
    ...
    { name: "table load", kind: "command", description: "Load a czfs Volume path into a table (directory sources must end with /); use SQL for advanced COPY INTO/OVERWRITE cases",
      arguments: [{ name: "name", required: true }, { name: "source", required: true }],
      options: [{ flags: "--using", required: false, takes_value: true, help: "Input format: csv, parquet, orc, bson (default csv)" }, { flags: "--header", required: false, takes_value: false, help: "Treat the first CSV row as column names" }, { flags: "--write", required: false, takes_value: false, help: "Allow loading only after explicit user approval" }],
