...
async function checkCdcPrereqs(
  ...
  // DM (Dameng) — not tested, based on standard DM CDC docs
...
// CDC pipeline lifecycle. The CDC operation functions (offlineCdcTask, start/stop/
// resync/pause/recoverCdcTables, listRealtimeTasks, listPipelineTables) are imported
// from @clickzetta/sdk — matches origin/main (they were briefly inlined here while the
// a2 SDK snapshot lagged; restored to the SDK once it was synced forward).
...
function registerCdcCommands(cdcYargs: Argv<GlobalArgs>): Argv<GlobalArgs> {
  const tableOp = (
    ...
    try {
      ...
      const ids = String(argv["table-ids"] ?? "").split(",").map((s) => s.trim()).filter(Boolean)
      if (!ids.length) {
