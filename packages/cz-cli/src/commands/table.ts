...
import { classifyExecError, execSql, getExecContext, isQueryResult, rowsToRecords, validateIdentifier } from "./exec.js"
            ...
            if (r.jobId) {
              try {
                const body = {
                  get_summary_request: {
                    account: { user_id: 0 },
                    job_id: { id: r.jobId, workspace: ctx.config.workspace, instance_id: ctx.instanceId() },
                    offset: 0,
                    user_agent: "",
                  },
                  user_agent: "",
