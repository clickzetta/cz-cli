...
function Cleanup-ExternalAgentSkill() {
  # External AI clients (Claude Code / Cursor / Codex) are no longer wired up by
  # auto-installing a skill. They now integrate via MCP — run \`cz-cli mcp init\`.
  # Remove any skill (current or deprecated aliases) left by older installers so
  # an upgrade doesn't leave a stale cz-cli skill behind.
  $AgentDirs = @(
    @(".claude", "skills"),
    @(".agents", "skills"),
    @(".kiro", "skills"),
  ...
  $LegacySkills = @("cz-cli", "czagent", "czcli", "cz-cli-v2")
