...
- [x] 1.2 Update `packages/opencode/src/cli/ui.ts` — replace the `wordmark` array to render "czcode" instead of "opencode"

## 2. Theme (additive only)

- [x] 2.1 Create `packages/opencode/src/cli/cmd/tui/context/theme/czcode.json` with ClickZetta brand colors (primary `#1677FF`, dark bg `#0D1117`, cool gray neutrals)
- [x] 2.2 Change default theme string from `"opencode"` to `"czcode"` in `packages/opencode/src/cli/cmd/tui/context/theme.tsx` (one-line change)

## 3. Home Screen (text-only changes, preserve layout)

- [x] 3.1 Replace `Logo` component body in `packages/opencode/src/cli/cmd/tui/component/logo.tsx` with static "czcode" text banner — keep file name and export name identical
- [x] 3.2 Update placeholder arrays in `packages/opencode/src/cli/cmd/tui/routes/home.tsx` to czcode-relevant suggestions
...
- [x] 4.1 Delete `packages/opencode/src/cli/cmd/tui/component/dialog-go-upsell.tsx` and remove imports; leave `// TODO(czcode): Add ClickZetta subscription dialog` in `app.tsx`
- [x] 4.2 Delete `packages/opencode/src/cli/cmd/tui/component/dialog-console-org.tsx` and remove imports; leave `// TODO(czcode): Add ClickZetta org picker` in `app.tsx`
...
- [x] 6.1 Update loading text in `packages/opencode/src/cli/cmd/tui/component/startup-loading.tsx` to reference "czcode"
