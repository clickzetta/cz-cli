...
### Requirement: Home screen SHALL display a static czcode brand mark
The `Logo` component in `logo.tsx` SHALL be replaced with a static "czcode" text banner. The file and export name SHALL remain identical to minimize upstream merge conflicts. No animation, glow, shimmer, or interactive effects SHALL execute.

#### Scenario: Home screen renders static brand
- **WHEN** the user is on the home screen
- **THEN** a static "czcode" text banner is displayed above the prompt
...
### Requirement: Home screen placeholders SHALL reflect czcode context
The prompt placeholder text arrays SHALL use czcode-appropriate suggestions.

#### Scenario: Placeholder text is czcode-branded
- **WHEN** the prompt is empty on the home screen
- **THEN** placeholder text references czcode or ClickZetta Lakehouse capabilities
...
### Requirement: Home footer SHALL display czcode version
The version string in the home footer SHALL use czcode branding. Directory and MCP status display SHALL remain unchanged.

#### Scenario: Footer shows czcode version
- **WHEN** the home footer renders
- **THEN** the version area shows "czcode" branding, not "opencode"
