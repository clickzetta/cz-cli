...
### Requirement: 将 cz-cli skill 注册到外部 agent 目录（先删除再安装）
...
#### Scenario: Windows PowerShell 原生注册外部 agent skill
