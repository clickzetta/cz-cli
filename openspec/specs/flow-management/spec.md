...
#### Scenario: flow group help signatures
- **WHEN** user runs `cz-cli task flow --help`
- **THEN** help lists `dag/create-node/remove-node/bind/unbind/node-detail/node-save/node-save-config/submit/instances`
- **AND** node operations use positional `TASK_NAME_OR_ID`
...
- **WHEN** user runs `cz-cli task flow create-node TASK_ID_OR_NAME --name transform --type python`
- **THEN** system creates a node in the flow task

#### Scenario: Remove flow node
- **WHEN** user runs `cz-cli task flow remove-node TASK_ID_OR_NAME --name transform`
...
#### Scenario: Bind help signature
- **WHEN** user runs `cz-cli task flow bind [OPTIONS] TASK_NAME_OR_ID`
- **THEN** command accepts required `--upstream` and `--downstream` options

#### Scenario: Unbind help signature
- **WHEN** user runs `cz-cli task flow unbind [OPTIONS] TASK_NAME_OR_ID`
- **THEN** command accepts required `--dependency-id` option
...
- **WHEN** user runs `cz-cli task flow node-detail TASK_ID_OR_NAME --node NODE_ID`
- **THEN** system returns node detail including content and defaults

#### Scenario: Save node content
- **WHEN** user runs `cz-cli task flow node-save TASK_ID_OR_NAME --node NODE_ID --content "..."`
- **THEN** system saves node content successfully

#### Scenario: Save node configuration
- **WHEN** user runs `cz-cli task flow node-save-config TASK_ID_OR_NAME --node NODE_ID`
...
- **WHEN** user runs `cz-cli task flow submit TASK_ID_OR_NAME`
