...
- **`--username`/`--password`, `--login-method`, `--login`** (explicit non-interactive credentials or portal-discovery signals) → delegate to the shared non-interactive/portal-discovery configuration flow (the same implementation as the `setup` alias), covering CI/agent and username/password portal discovery. The profile name defaults to `default` when `--name` is absent — for every flag in this group, including `--login-method`/`--login`, where the name was previously the literal string `undefined`; on a machine that already has a `default` profile such a run therefore now ends in `PROFILE_EXISTS` from the shared writer instead of creating a garbage-named profile. It matches the `setup` alias and the `--credential` path; a COMPLETE credential set (`--username` + `--password` + `--account-name`) with no name and an existing `default` profile fails fast with `PROFILE_EXISTS` rather than after the portal round trip, while an incomplete set still receives the flow's `SETUP_INPUT_REQUIRED` step so a wrapper can continue.
- **`--pat <token>` as the only credential** (on `login` and equally on the deprecated `setup` alias, which runs the same flow) → error `PAT_NOT_A_LOGIN` (exit 2) pointing at `cz-cli profile create <name> --pat <token>`. A PAT is a stored profile credential, not a sign-in: the shared configuration flow reads no `pat` at all, so delegating it dropped the PAT silently and then demanded a username. When `--pat` accompanies another credential the login proceeds with that credential (the PAT is ignored) and a notice is written to stderr, so a wrapper that forwards `--pat` unconditionally keeps working.
...
- **`llm.json` is not written**, unless `--refresh-llm` asks for it. Its `api_key` may be the gateway virtual key that the quota-exhaustion flow swapped into the same entry, and nothing here can distinguish that from a complimentary key that was revoked server-side — so the overwrite is an explicit request rather than a default, and `--refresh-llm` is the recovery path for a dead key (honoured on every provisioning path, including the one that enumerated nothing). The flag exists because the llm entry records no provenance for its `api_key`: `llm/key-provision.ts` swaps a gateway virtual key in place without a marker, so login cannot distinguish it from a revoked complimentary key. Recording that provenance would let login refresh a complimentary key unconditionally and never touch a provisioned one — a tracked follow-up, not a property of this design. Without the flag an entry the user deleted is not recreated either; `agent llm add` / `ai-gateway key create` also work. Because the default skip is unconditional, a session that never got an entry (its first login carried no `apiKey`, or it predates LLM auto-config) would stay entry-less silently, so a re-login that finds no entry under its session id — nor under the legacy `<base>_0` key that predates the rename — SHALL report a warning naming those commands.
- **Existing profiles keep what the USER owns** — `schema`, `vcluster`, and `header.Cookie` (which cannot then shadow the refreshed token on a row with no pin: every row a login touches has `auth_type` re-asserted to `oauth` when absent, and a non-cookie pin drops the Cookie header at connection-resolution time). A row EXPLICITLY pinned `auth_type = "cookie"` is the exception — the pin is the user's and is not overwritten, so that profile keeps authenticating with its cookie and never reads the refreshed token; the login SHALL report those rows in a warning naming the field to clear rather than reporting an unqualified success — while what the SERVER owns is refreshed: `service` (the region business host) and `protocol` from the authenticated connection, plus `aimeshEndpointBaseUrl` and the `account_id`/`account_name`/`user_id` identity from userinfo. Neither of the latter is a preference, and no other command rewrites them, so freezing them would make a region or endpoint move impossible to pick up by logging in again. `instance`/`workspace` are what a row is matched on, so re-writing them is a no-op. The `oauth` pointer and (when absent) `auth_type` are re-asserted so the refreshed token resolves.
...
- **Only genuinely new `instance × workspace` combinations are added**, named `<base>_<max(N) + 1>`, which guarantees no collision with a live row (a name freed by `profile delete` in the middle of the range stays free; one freed at the top of the range can be handed to a different connection, since nothing persists the high-water mark). Combinations are matched to existing profiles by connection content rather than by enumeration index, so `<base>_N` does not shift when the server returns the combinations in another order. A row is only matched when its `oauth` pointer names THIS session (the one exception is the bare `<name>` profile of the zero-combos path, which `login <name>` fills or replaces by name as it always has — superseding `setup` is what that means; the `<name>_N` rows are login's own creations, so adopting one it did not create would be a surprise): a row belonging to another session, or a hand-written row with no pointer, keeps its name reserved but is never written to, since stamping `oauth`/`auth_type` onto it would change the credential it authenticates with. A connection the server reports twice yields one profile, reported once.

The success output SHALL carry `relogin` and `profiles_created` (what this run created, which on a re-login is normally empty). `llm_configured` SHALL remain a plain boolean and SHALL be `false` on a re-login: a truthy "not attempted" value would tell every consumer written as `if (llm_configured)` that the LLM is ready after a run that never looked, and omitting the key would move the ambiguity onto the parser — `false` fails safe. Which kind of `false` it is SHALL be reported in its own key, `llm_configuration`: `"written"`, `"skipped_relogin"`, or `"no_api_key"`. That value SHALL be reported by the provisioning code rather than re-derived by the command from its own flags, since only the provisioner knows which branch ran. `profiles` lists every profile this session owns, ordered by `N` with the bare `<name>` row after the numbered ones (callers read `profiles[0]`, and that row may carry no `instance`), not only the ones this run created or matched — a re-login during a per-instance enumeration failure must not report a shrunken list with nothing having been deleted. `default_profile` keeps its established meaning — what `profiles.toml` says, i.e. the profile bare commands will use. The session-scoped answer (the user's selection when it names a profile this session owns, else the session's first profile) is reported separately as `session_default_profile`, because a re-login normally does not write `default_profile` and the two can therefore legitimately differ. Rows this session owns are identified by their `oauth` pointer, not by name shape, which is what makes the bare `<base>` row of a zero-combos login part of the session rather than invisible to it.
...
- **AND** the reported `default_profile` is the one on disk, not this run's first touched profile
...
- **AND (boundary)** `service` is refreshed ONLY on the row userinfo describes, since it is the region host of an instance and this path knows only the default instance's — writing it to every row would move another region's profile onto the wrong host — and never when it is the OAuth entry host standing in for a region host (no `gatewayMapping` in userinfo)
- **AND** a row that carries NO `instance` is filled from userinfo, since filling an empty field is not overwriting a user edit: that row is the bare `<name>` shape whose documented remedy is "provision an instance, then log in again", and this branch is how that remedy arrives when the enumeration is still failing
