...
- **`default_profile` is not written.** It is reported only when it names one of this session's profiles; otherwise the session's first profile is reported, since `default_profile` is a single global string with nothing tying it to a session.
...
The success output SHALL carry `relogin` and `profiles_created` (what this run created, which on a re-login is normally empty), and SHALL omit `llm_configured` on a re-login, since reporting `false` for something the command deliberately did not attempt reads as a failure. `profiles` lists this session's profiles, ordered by `N`, not only the ones just created.
...
- **AND** the llm entry's `api_key`, the edited profile fields, and `default_profile` are all unchanged on disk
