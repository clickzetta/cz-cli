...
`DESCRIBE HISTORY`命令用于获取表、动态表或物化视图的历史记录信息。通过这些历史记录，用户可以执行回滚操作，或者查询特定时间点的表数据。是否可以查看的对象操作历史依赖于数据的[保留周期](TIMETRAVEL.md)。
...
执行该命令后，将返回`public.event_gettime`动态表的历史记录，如下所示：
