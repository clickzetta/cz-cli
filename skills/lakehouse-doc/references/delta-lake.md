## DELTA LAKE
Delta Lake 是一个开源的大数据框架，用于提供数据湖上的可靠性和性能。它建立在 Apache Spark 和其他大数据处理引擎之上，为存储在数据湖中的数据提供了一种结构化存储层，使得数据湖可以像数据仓库一样易于使用。Lakehouse 提供了外部表的方式来访问该格式数据。
...
外部表和内部表权限点相同。由于外部表无法执行 INSERT、UPDATE、TRUNCATE、DELETE、UNDROP 等操作，所以没有对应的操作权限点。
