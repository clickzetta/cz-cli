...
`SHOW CREATE DYNAMIC TABLE`命令Lakehouse使用`SHOW CREATE TABLE`来代替。`SHOW CREATE TABLE`命令用于获取指定表、外部表、物化视图、动态表或视图的创建语句。
...
* `object_name`：指定要查询的数据库对象名称。这可以是表、外部表、物化视图、动态表或视图的创建语句。
