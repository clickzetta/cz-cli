...
排序键用于定义文件内数据的排序方式。对于需要按排序键进行排序的查询，预先将数据按排序键组织可以提高查询性能。

### 使用排序键的注意事项

- 可以为排序键指定升序（ASC）或降序（DESC）。
- 虽然排序键可以提高查询性能，但对大量数据进行插入时排序可能会消耗较多资源。
...
在这个示例中，创建了一个名为`sales_data`的表，数据将根据`product_id`列的Hash值被分散到50个分桶中。同时，分桶内的数据将按照`sale_date`列的降序进行排序。
